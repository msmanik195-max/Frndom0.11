...
class StoryRepository(private val context: Context) {
    ...
    init {
        ...
        startPeriodicExpirationCleaner()
    }

    private fun startPeriodicExpirationCleaner() {
        scope.launch {
            while (isActive) {
                try {
                    delay(5000)
    ...
    fun cleanExpiredStories() {
        val now = System.currentTimeMillis()
        val current = _storiesFlow.value
        val expired = current.filter { it.expiresAt <= now }
        if (expired.isNotEmpty()) {
            val remaining = current.filter { it.expiresAt > now }
            _storiesFlow.value = remaining
            saveLocalStories(remaining)
            // Permanently remove expired stories from Firebase Realtime Database
            expired.forEach { story ->
                try {
                    dbRef?.child(story.id)?.removeValue()
                    Log.d("StoryRepository", "Permanently auto-deleted expired story from database: ${story.id}")
                } catch (e: Exception) {
                    Log.w("StoryRepository", "Error deleting expired story ${story.id}: ${e.message}")
                }
            }
        }
    }
                    ...
                    for (child in snapshot.children) {
                        try {
                            ...
                            if (expiresAt > now) {
                                ...
                                // Automatically and permanently remove expired story from Firebase database
                                try {
    ...
    fun createStory(story: StoryItem) {
        ...
        val customDurationMs = adminRepo.getStoryExpiryDurationMs()
        val now = System.currentTimeMillis()
        val calculatedExpiresAt = now + customDurationMs

        val storyWithId = if (story.id.isBlank()) story.copy(id = UUID.randomUUID().toString()) else story
        val newStory = storyWithId.copy(
            createdAt = now,
        ...
        try {
            ContentLimitManager.getInstance(context).recordContentCreated(newStory.userId, false, "story")
        } catch (e: Exception) {
            Log.w("StoryRepository", "Error recording daily story limit count: ${e.message}")
        }

...
class UserRepository(private val context: Context) {
    ...
    fun getLastActiveUserUid(): String? {
        val prefs = context.getSharedPreferences("frndom_user_cache", Context.MODE_PRIVATE)
        return prefs.getString("last_active_user_uid", null)
    }

    fun setLastActiveUserUid(uid: String) {
        val prefs = context.getSharedPreferences("frndom_user_cache", Context.MODE_PRIVATE)
        prefs.edit().putString("last_active_user_uid", uid).apply()
    }

    fun clearLastActiveUserUid() {
        val prefs = context.getSharedPreferences("frndom_user_cache", Context.MODE_PRIVATE)
        prefs.edit().remove("last_active_user_uid").apply()
    }

    fun saveLocalUserProfile(profile: UserProfile) {
        val enriched = enrichProfileWithVerification(profile)
        setLastActiveUserUid(enriched.uid)
    ...
    fun getCurrentUser(): UserProfile? {
        ...
        val lastActiveUid = getLastActiveUserUid()
        if (!lastActiveUid.isNullOrBlank()) {
            val cached = getLocalUserProfile(lastActiveUid)
            if (cached != null) return cached
        }

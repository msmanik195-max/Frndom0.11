...
class MediaUploadService(
    ...
    suspend fun uploadMedia(
        ...
        return if (mimeType.startsWith("video/")) {
            uploadVideoUri(uri, folder)
        } else {
            uploadImageUri(uri, folder)
        }
    }

    companion object {
        @Volatile
        private var INSTANCE: MediaUploadService? = null

        fun getInstance(context: Context): MediaUploadService {
            return INSTANCE ?: synchronized(this) {
                INSTANCE ?: MediaUploadService(
                    context.applicationContext,
                    StorageRepository(context.applicationContext)
                ).also { INSTANCE = it }
            }
        }
    }
